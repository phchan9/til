...
2016.06.14 update:   
Sometimes we need to calculate the size or height for our cell which is ready to show but its calculation depends on the constraints on IB. Then, we load its xib to figure out the exact size or height in delegate methods (e.g. `collectionView:layout:sizeForItemAtIndexPath:`, `tableView:heightForRowAtIndexPath:`); however, it isn't an efficient way to dequeue cell again in these delegate methods to get the ready-to-show cell which we already dequeue in methods like `tableView:cellForRowAtIndexPath:
` or `collectionView cellForItemAtIndexPath:`. Therefore, this is the reason that we can preload the cell from xib in the beginning in order to calculate the height or size by reusing this preloaded cell. In my fourth reference, it pointed out this idea with his example.
...
* [Post: Variable-Sized Items in UICollectionView](https://www.cocoanetics.com/2013/08/variable-sized-items-in-uicollectionview/)
